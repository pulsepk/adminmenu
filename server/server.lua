...
RegisterNetEvent('qb-admin:server:inventory', function(player)
    ...
    if isAdmin(xPlayer) then
    TriggerClientEvent('qb-adminmenu:client:openinv',src, player.id)
    else
        BanPlayer(src)
    end
end)
...
ESX.RegisterCommand('admin', 'admin', function(xPlayer, args, showError)
	xPlayer.triggerEvent('qb-admin:client:openMenu', source)
end, false)

ESX.RegisterCommand('fix', 'admin', function(xPlayer)
    xPlayer.triggerEvent('qb-adminmenu:adminfix')
end, false)

ESX.RegisterCommand('dvspawn', 'admin', function(xPlayer)
    local ped = GetPlayerPed(xPlayer.source)
    local pedVehicle = GetVehiclePedIsIn(ped, false)

    if DoesEntityExist(pedVehicle) then
        DeleteEntity(pedVehicle)
        TriggerClientEvent('esx:showNotification', xPlayer.source, 'Vehicle deleted')
    else
        TriggerClientEvent('esx:showNotification', xPlayer.source, 'No vehicle to delete')
    end
end, false)

ESX.RegisterCommand('maxmods', 'admin',function(xPlayer)
    local ped = GetPlayerPed(xPlayer.source)
    local pedVehicle = GetVehiclePedIsIn(ped, false)
    if DoesEntityExist(pedVehicle) then
    local src = source
    xPlayer.triggerEvent('qb-admin:client:maxmodVehicle', src)
        ...
        TriggerClientEvent('esx:showNotification', xPlayer.source, 'No vehicle to max mod')
    end
end, false)
...
QBCore.Commands.Add('staffchat', Lang:t("commands.staffchat_message"), {{name='message', help='Message'}}, true, function(source, args)
    ...
    for i = 1, #plrs, 1 do
        local plr = plrs[i]
        if HasPermission(plr, 'admin') or IsPlayerAceAllowed(plr, 'command') then
...
QBCore.Commands.Add('reportr', Lang:t("commands.reply_to_report"), {{name='id', help='Player'}, {name = 'message', help = 'Message to respond with'}}, false, function(source, args)
    ...
    if not HasPermission(src, 'admin') or IsPlayerAceAllowed(src, 'command') ~= 1 then return end
...
QBCore.Commands.Add('kickall', Lang:t("commands.kick_all"), {}, false, function(source, args)
    ...
    if src > 0 then
        ...
        if HasPermission(src, 'god') or IsPlayerAceAllowed(src, 'command') then
    ...
    local ped = ESX.GetPlayerFromId(source)
        local reason = table.concat(args, ' ')
            ...
            if reason and reason ~= '' then
                ...
                TriggerClientEvent('esx:showNotification', src, "No reason specified")
            end
end)

ESX.RegisterCommand('vector2', 'admin',function(xPlayer)
    local src = source
    xPlayer.triggerEvent('qb-admin:client:copyToClipboard', src, 'coords2')
end)
ESX.RegisterCommand('vector3', 'admin',function(xPlayer)
    local src = source
    xPlayer.triggerEvent('qb-admin:client:copyToClipboard', src, 'coords3')
end)
ESX.RegisterCommand('vector4', 'admin',function(xPlayer)
    local src = source
    xPlayer.triggerEvent('qb-admin:client:copyToClipboard', src, 'coords4')
end)
ESX.RegisterCommand('heading', 'admin',function(xPlayer)
    local src = source
    xPlayer.triggerEvent('qb-admin:client:copyToClipboard', src, 'heading')
end)
