...
RegisterNetEvent('qb-admin:client:toggleBlips', function()
    if not ShowBlips then
        ShowBlips = true
        NetCheck1 = true
        ESX.ShowNotification(Lang:t("success.blips_activated"), "success")
    else
        ShowBlips = false
        ESX.ShowNotification(Lang:t("error.blips_deactivated"), "error")
    end
end)

RegisterNetEvent('adminmenu:client:toggleNames', function()
    if not ShowNames then
        ShowNames = true
        NetCheck2 = true
        ESX.ShowNotification(Lang:t("success.names_activated"), "success")
    else
        ShowNames = false
        ESX.ShowNotification(Lang:t("error.names_deactivated"), "error")
    end
end)
