...
-- Weapons list
for _,v in pairs(ESX.GetConfig().Weapons) do
    menu15:AddButton({icon = '🎁',
        label = v.label ,
        value = v.value ,
        description = Lang:t("desc.spawn_weapons_desc"),
        select = function(_)
            TriggerServerEvent('adminmenu:giveWeapon', v.name)
        end
    })
end
...
local function OpenPermsMenu(permsply)
    ESX.TriggerServerCallback('adminmenu:server:getrank', function(rank)
        if rank then
            ...
            menu11:AddButton({
                ...
                select = function(_)
                    if selectedgroup ~= 'Unknown' then
                        TriggerServerEvent('adminmenu:server:setPermissions', permsply.id, selectedgroup)
    ...
    for _, v in ipairs(elements) do
        menu8:AddButton({icon = v.icon,label = v.label,value = v,description = v.description,select = function(btn)
            ...
            if GetResourceState('cd_easytime') ~= 'missing' then
                TriggerServerEvent('cd_easytime:ForceUpdate', {weather = selection.value})
            end
            if GetResourceState('vSync') ~= 'missing' then
                ExecuteCommand('weather '..selection.label)
            end
...
menu3_server_time:On("select", function(_, value)
    ...
    if GetResourceState('cd_easytime') ~= 'missing' then
        TriggerServerEvent("cd_easytime:ForceUpdate", {hours = tonumber(value)})
    end
    if GetResourceState('vSync') ~= 'missing' then
        ExecuteCommand('time '..value..' '..00)
    end
...
CreateThread(function()
    ESX.TriggerServerCallback('adminmenu:retrievevehicles', function(response)
        vehicles = response
    end)
end)
...
--[[ CreateThread(function()
    ESX.TriggerServerCallback('adminmenu:getvehicles', function(Vehicles)
        for k, v in pairs(Vehicles) do
...
local function CopyToClipboard(dataType)
    ...
    if dataType == 'coords2' then
        ...
        local x = ESX.Math.Round(coords.x, 3)
        local y = ESX.Math.Round(coords.y, 3)
        SendNUIMessage({
    ...
    elseif dataType == 'coords3' then
        ...
        local x = ESX.Math.Round(coords.x, 3)
        local y = ESX.Math.Round(coords.y, 3)
        local z = ESX.Math.Round(coords.z, 3)
        SendNUIMessage({
    ...
    elseif dataType == 'coords4' then
        ...
        local h = ESX.Math.Round(heading, 3)
        SendNUIMessage({
    ...
    elseif dataType == 'freeaimEntity' then
        ...
        if entity then
            ...
            local x = ESX.Math.Round(entityCoords.x, 3)
            local y = ESX.Math.Round(entityCoords.y, 3)
            local z = ESX.Math.Round(entityCoords.z, 3)
            local rotX = ESX.Math.Round(entityRotation.x, 3)
            local rotY = ESX.Math.Round(entityRotation.y, 3)
            local rotZ = ESX.Math.Round(entityRotation.z, 3)
            ...
            ESX.ShowNotification(Lang:t("success.entity_copy"), "success")
        else
            ESX.ShowNotification(Lang:t("error.failed_entity_copy"), "error")
        end
    end
end
    ...
    CreateThread(function()
        while vehicleDevMode do
            ...
            if IsPedInAnyVehicle(ped, false) then
                ...
                Draw2DText(string.format(Lang:t("info.eng_health") .. '~b~%s~s~ | ' .. Lang:t("info.body_health") .. '~b~%s~s~', ESX.Math.Round(eHealth, 2), ESX.Math.Round(bHealth, 2)), 4, {255, 255, 255}, 0.4, x + 0.0, y + 0.075)
            end
        end
    end)
end
...
entity_view_distance:On("select", function(_, value)
    ...
    ESX.ShowNotification(Lang:t("info.entity_view_distance", {distance = value}))
end)
